...
export class IncomeService {
  ...
  async rebuildVenue(venueId: string) {
    ...
    for (const shift of shifts) {
      ...
      // allocation for this worker on this business date (approved batches only)
      ...
      const allocatedUsdCents = allocation?.netAllocatedUsdCents ?? 0;

      const finalizedPayout = allocation?.payouts.find((p) => p.status === "FINALIZED");
      const paidUsdCents = finalizedPayout?.amountUsdCents ?? 0;
      const payoutRail = finalizedPayout?.rail ?? allocation?.payoutRail ?? null;

      // payroll record covering this business date
      ...
      const payrollReportedUsdCents = payroll?.reportedTipUsdCents ?? 0;
